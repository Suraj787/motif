"""OII command-line interface. Offline by default; transparent output.

Run `python -m oii --help`. Internet retrieval only happens with an explicit
...
def cmd_source(args) -> int:
    ...
    if args.action == "scan":
        if not args.path:
            _print("usage: oii source scan <path>")
            return 2
    ...
    if args.action == "discover":
        ...
        for h in pol.get("allowlist", []):
            _print(f"  - {h}")
        _print("\nUse `oii source retrieve --refresh <url>` to pull into quarantine (allowlisted only).")
        return 0
    ...
    if args.action in ("retrieve",):
        if not args.refresh:
            ...
            return 3
        _print("retrieve --refresh: would pin version+SHA-256 into .oii/quarantine and scan before review.")
        _print("(Network retrieval is intentionally not performed in this offline build.)")
        return 0

    if args.action in ("approve", "reject"):
        _print(f"source {args.action}: moves a reviewed item between .oii/reviewed and "
               f".oii/{'approved' if args.action == 'approve' else 'rejected'} after human sign-off.")
        return 0
...
def cmd_component(args) -> int:
    ...
    if args.action == "plan-install":
        if not args.target:
            _print("usage: oii component plan-install <id> --target <dir>")
            return 2
    ...
    if args.action == "install":
        if not args.target:
            _print("usage: oii component install <id> --target <dir> --approve")
            return 2
    ...
    if args.action == "rollback":
        if not args.target:
            _print("usage: oii component rollback --target <dir>")
            return 2
...
def cmd_generate_index(_args) -> int:
    ...
    lines = ["# Registry index", "", "_Generated by `python -m oii generate-index`._", ""]
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="oii", description="Open Interaction Intelligence CLI (offline by default)")
    p.add_argument("--version", action="version", version=f"oii {__version__}")
