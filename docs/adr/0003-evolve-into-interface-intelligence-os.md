# ADR 0003, Evolve Motif from interaction foundation into a full platform
...
The Motif specification treats that foundation as **one engine** (the
...
1. **Evolve in place, on a branch.** Build Motif on the
   `motif` branch so `main` keeps the intact, validated Motif v1.0.0.
...
4. **Target a strong, honest v2.0.0.** Implement functioning foundations for the new
