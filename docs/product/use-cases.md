...
4. **State Completeness** requires empty/loading/error/permission-denied/overflow states, *planned*.
5. **Implementation** emits framework-correct, own-your-source code (e.g. Frappe-Vue), *implemented (adapters)*.
...
- Originality / Aesthetic-Convergence Detector flags default-template signatures (stock
  gradient-on-dark hero, generic component clichés) and nudges toward intentional design., *experimental*.
