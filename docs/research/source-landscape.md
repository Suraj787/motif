...
**CSS Transitions / Animations / Scroll-driven Animations, Web Animations API, View Transitions API, Intersection Observer, Resize Observer, Pointer Events, SVG (SMIL+CSS), Canvas 2D, WebGL, WebGPU.** Browser standards → `redistributable`,
documented via MDN ([ledger #8](./research-ledger.md)).

### 3. Animation engines
**Motion (Framer Motion), Motion for Vue, @vueuse/motion, Svelte Motion, react-spring, Anime.js, AutoAnimate, Theatre.js (Apache core), GSAP.** Mostly MIT → `redistributable`;
...
**PixiJS, Konva, Fabric.js, Paper.js, tsParticles, Vanta.js, regl, OGL (Unlicense), p5.js.** Mostly MIT → `redistributable`; **p5.js is LGPL‑2.1 → `adaptable-concept`**.
...
**Magic UI, React Bits, Motion Primitives, Cult UI, Kokonut UI, SmoothUI, Animata, Fancy Components, GodUI** (MIT but copy-paste → mostly `adaptable-concept`); **Aceternity UI
(unknown per-component), Uiverse (community-submitted), Hover.dev (proprietary), Animate UI / Vue Bits / Svelte Bits (MIT + Commons Clause, source-available)** →
